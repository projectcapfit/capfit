...
class UserGameDataViewModel @Inject constructor(
    ...
    fun addFriend(friendUid: String) {
        viewModelScope.launch {
            ...
            try {
                ...
                if (success) {
                    val current = _userGameData.value?.friendsList ?: emptyList()

                    if (!current.contains(friendUid)) {
                        _userGameData.value =
                            _userGameData.value?.copy(
                                friendsList = current + friendUid
                            )
                    }
                } else {
    ...
    fun sendFriendRequest(receiverUid: String) {
        viewModelScope.launch {
            userGameDataManager.sendFriendRequestNotification(receiverUid)
        }
    }
    ...
    fun searchUsers(query: String) {
        viewModelScope.launch {
            ...
            try {
                ...
                val myUid = userGameData.value?.uid
                val myFriends = userGameData.value?.friendsList ?: emptyList()

                _searchResults.value = results.filter {
                    it.uid != myUid && !myFriends.contains(it.uid)
                }
    ...
    fun loadFriends() {
        viewModelScope.launch {
            _friends.value = userGameDataManager.loadFriends() ?: emptyList()
        }
    }

    fun checkAndResetStreak(todayDate: String) {
        viewModelScope.launch {
            userGameDataManager.checkAndResetStreakIfNeeded(todayDate)
        }
    }
